...
`ASSISTANT_SECTION_MASTER`, or a distinct `HELPER` role with narrower access?
...
- **Use `ASSISTANT_SECTION_MASTER`** — rejected as the default answer. It grants
...
- Until a role exists for them, an over-18 Rover helping with a Pack must either
  be given `ASSISTANT_SECTION_MASTER` as an explicit, recorded decision under
